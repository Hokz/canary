...
**HOD-05 re-investigation**: confirmed the catalog system's `description` field *can* support dynamic per-player text — `data-otservbr-global/lib/core/quests/catalog/034_wrath_of_the_emperor.lua` already uses `description = function(player) return string.format("...%d...", player:getStorageValue(...)) end` for exactly this "X of Y" pattern. So the *mechanism* to render "You have gathered X of 5 charges" dynamically already exists and is proven safe elsewhere in this codebase. **This does not change the deferral decision** — the blocker isn't the catalog system's capability, it's that the underlying charges storage this text would read doesn't exist (see [[05_HOD_BOSS_MECHANICS_CONTRACT]] "destructive charges" finding). Wiring exact text to a storage that's never written would silently always show "0 of 5 charges," which is worse than not showing it at all.

**Not implemented in HOD-03/05**, for two reasons:
